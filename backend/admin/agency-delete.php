...
if (!isset($_GET['q'])) {
  ...
  $sql = "DELETE FROM `agencyregisterrequest` WHERE id=$id";
  $result = mysqli_query($conn, $sql);
  if (!$result) {
    header('Location: ./index.php?msg=We encountered an error with the Deleteing Data please try again&&msg_class=danger');
  } else {
    header('Location: ./index.php?msg=Great this Request is Permanently Deleted and No More Live on Website&&msg_class=success');
  }
}
